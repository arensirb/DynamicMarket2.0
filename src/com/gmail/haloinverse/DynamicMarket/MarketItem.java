...
public class MarketItem extends ItemClump {
	...
	public double stock;      	//s		// current stock level of item
	public boolean canBuy;     //cb	// true if can be purchased, false if not
	public boolean canSell;    //cs	// true if can be sold, false if not
	private double volatility; 	//v		// % change in price per 1 stock bought/sold, * intScale
	...
	public double stockLowest;		//sl	// minimum stock at which purchases will fail
	public double stockHighest;	//sh	// maximum stock at which sales will fail
	public double stockFloor;		//sf	// minimum stock level possible
	public double stockCeil;		//sc	// maximum stock level possible
	public double priceFloor;		//pf	// minimum price <- applies to basePrice, without salesTax
	public double priceCeil;		//pc	// maximum price
			...
			for (int i = 1; i <= initData.length - 1; i++)
			{
				...
				else
				{
					...
					{	// Try to parse it as a plain integer.
						...
						curVal = Double.parseDouble(initData[i]);
					}
					catch (NumberFormatException ex)
				...
				if (i == 2)
				{
					if ((setUntaggedBase) && (curTag == null))
					{
						if (curVal != -1)
						{
							...
							if (this.basePrice != 0)
							{
								//this.salesTax = (int) Math.round((1 - ((float)curVal / basePrice)) * 100);
								this.salesTax = (int) Math.round((1 - (curVal / basePrice)) * 100);
								//maskData.salesTax = 1;
							}
							else
							{
	...
	public double getRenormedPrice(double newStock)
	{
		// Calculate what the renormalized base price would be when shifting from the current stock level to newStock.
		double oldBuyPrice = getStockPrice(this.stock);
		//return Math.round(oldBuyPrice / Math.pow(getVolFactor(), -rangeCrop(newStock, stockFloor, stockCeil)));
		return oldBuyPrice / Math.pow(getVolFactor(), -rangeCrop(newStock, stockFloor, stockCeil));
	}
	...
	public static double rangeCrop(int value, int minVal, int maxVal)
	{
	...
	public double getBatchPrice(double stockFloor2, double endStock)
	{
		...
		double lowStock = Math.min(stockFloor2, endStock);
		double highStock = Math.max(stockFloor2, endStock);
		double numTerms = highStock - lowStock + 1;
	...
	public double getBuyPrice(int numBundles)
	{
		...
		return getBatchPrice(stock, stock - numBundles + 1);
	}
	...
	private double getStockPrice(double stockLevel) 
	{
	...
	public void setInverseVolatility(double newIVol)
	{
	...
	public static int iVolToVol(double invVol)
	{
		// Converts inverse volatility to volatility.
		if (invVol == Double.MAX_VALUE)
			return 0;
		else
	...
	public String formatBundleCount(double numBundles)
	{
		if (numBundles == 1)
			return (Double.toString(count));
		if (count == 1)
			return (Double.toString(numBundles));
		return (Double.toString(count) + "x" + Double.toString(numBundles));
	}
	...
	public ArrayList<String> infoStringFull()
	{
		...
		returnList.add("{}Stock:{BKT}[{PRM}" + (int)stock + "{BKT}]{} CanBuy:{BKT}[{PRM}" + (getCanBuy(1)? getBuyPrice(1) : "-") + "{BKT}]{} CanSell:{BKT}[{PRM}" + (getCanSell(1)? getSellPrice(1) : "-") + "{BKT}]");
	...
	public String intFormat(double thisInt)
	{
		...
		if (thisInt == Integer.MIN_VALUE)
			return "-INF";
		return Integer.toString((int)thisInt);
	}
	...
	private double quickParse(String intString)
	{
	...
	public MarketItem(String csvString, MarketItem defaults, String shopLabel, boolean isCSV)
	{
		...
		if (!inputParams[0].isEmpty())
			itemId = (int)quickParse(inputParams[0]);
		if (!inputParams[1].isEmpty())
			subType = (int)quickParse(inputParams[1]);
		if (!inputParams[2].isEmpty())
			count = (int)quickParse(inputParams[2]);
		...
		if (!inputParams[16].isEmpty())
			jitterPerc = (int)quickParse(inputParams[16]);
		if (!inputParams[17].isEmpty())
			driftOut = (int)quickParse(inputParams[17]);
		if (!inputParams[18].isEmpty())
			driftIn = (int)quickParse(inputParams[18]);
		if (!inputParams[19].isEmpty())
			avgStock = (int)quickParse(inputParams[19]);
		if (!inputParams[20].isEmpty())
			itemClass = (int)quickParse(inputParams[20]);
