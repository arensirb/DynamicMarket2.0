...
public class iListen extends PlayerListener
{
	...
	private void delta_balance(String name, double amount) //throws InvalidTransactionException
	{
		if (!DynamicMarket.econLoaded)
			return;
		if ((name != null) && (!name.isEmpty()))
	...
	//TODO make sure it works okie dokie :D
	public boolean shopRestock(String itemString, Messaging message, String shopLabel) 
	{
		ItemClump restocked = new ItemClump(itemString, plugin.db, shopLabel);
		String restockedItemName = null;
 
		if (restocked.itemId == -1) {
			message.send(plugin.shop_tag + "{ERR}Unrecognized item name or ID.");
			return false;
		}
	
		MarketItem itemToRestock = plugin.db.data(restocked, shopLabel);
		
		if (itemToRestock == null)
		{
			message.send(plugin.shop_tag + "{ERR}Item {PRM}" + restocked.getName(plugin.db, shopLabel) + "{ERR} not found in market.");
			return false;
		}

		restockedItemName = itemToRestock.getName();
		if (restockedItemName == null)
			restockedItemName = "<Unknown>";
		
		if (plugin.db.restock(restocked, shopLabel))
		{
			message.send(plugin.shop_tag + "Item " + restockedItemName + " was restocked.");
			return true;
		}	
		else
		{
			message.send(plugin.shop_tag + "Item " + restockedItemName + " {ERR}could not be restocked.");
			return false;
		}
	}
	...
	public boolean parseCommand(CommandSender sender, String cmd, String[] args, String shopLabel, String accountName, boolean freeAccount)
	{
		...
		if (cmd.toLowerCase().equals("shop")) {
			...
			if ((args.length == 1) && (!(Misc.isAny(subCommand, new String[] {"restock","add","list", "-l", "help", "-?", "idata", "reset", "reload", "exportdb", "importdb"})))) {	
				if(!plugin.db.inShop(args[0])) {
					message.send(plugin.shop_tag + "{ERR}Unrecognized item name, or not in shop.");
				 return false;
				}
			...
			if (subCommand.equalsIgnoreCase("restock")) {
				if (!(hasPermission(sender,"admin")))
				{
					message.send("{ERR}You do not have permission to restock the shop.");
					return false;
				}
				return shopRestock(Misc.combineSplit(1, args, " "), message, shopLabel);
			}
	...
	public String getCurrencyName(double amountOfCurrency)
	{
